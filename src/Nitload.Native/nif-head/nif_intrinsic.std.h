#ifndef NIF_INTRINSIC_CALLCONV_STD
#define NIF_INTRINSIC_CALLCONV_STD
...
#define NIF_STD_TARGET_ATTR(T_arch, T_feature) NIF_STD_TARGET_ATTR_IMPL(T_arch, T_feature)
...
#define NIF_MK_PROC_DECL(T_type, P_name) INTERNAL void NIF_DECL P_name(MAT_TASK(T_type) const *task)
#define NIF_MK_PROC_DECL_STD(T_type, T_arch, T_feature, T_op)                                   \
        INTERNAL NOINLINE NIF_STD_TARGET_ATTR(T_arch, T_feature) void NIF_DECL MK_PROC_NAME_STD( \
...
#define NIF_SK_PROC_DECL_STD(T_type, T_arch, T_feature, T_op)                                   \
        INTERNAL NOINLINE NIF_STD_TARGET_ATTR(T_arch, T_feature) void NIF_DECL SK_PROC_NAME_STD( \
...
#define NIF_STD__PROBE_PRINT(x) _SN_STD__PROBE_##x
#define NIF_STD__PROBE(x) NIF_STD__PROBE_PRINT(x)
...
#define _SN_MK_EXPAND_RIGHT_ANS                                                             \
        NIF_MAT_KERNEL_LOCAL const *right = (NIF_MAT_KERNEL_LOCAL const *)task->buffer_1.ptr; \
        NIF_MAT_KERNEL_LOCAL *restrict ans = (NIF_MAT_KERNEL_LOCAL *)task->buffer_2.ptr;\
        _SN_MK_EXPAND_BUFFER_SIZE(1); \
        _SN_MK_EXPAND_BUFFER_SIZE(2);

#define _SN_MK_EXPAND_LEFT_RIGHT_ANS                                                        \
        NIF_MAT_KERNEL_LOCAL const *left = (NIF_MAT_KERNEL_LOCAL const *)task->buffer_0.ptr;  \
        NIF_MAT_KERNEL_LOCAL const *right = (NIF_MAT_KERNEL_LOCAL const *)task->buffer_1.ptr; \
        NIF_MAT_KERNEL_LOCAL *restrict ans = (NIF_MAT_KERNEL_LOCAL *)task->buffer_2.ptr;\
...
#define __mk_param_std(param, T)                               \
        typedef MAT_KERNEL(T) NIF_MAT_KERNEL_LOCAL;             \
...
#define __sk_param_std(T)                          \
        typedef MAT_KERNEL(T) NIF_MAT_KERNEL_LOCAL; \
        (void)scalar
#endif // !NIF_INTRINSIC_PARAM_EXPAND
