...
#include <immintrin.h>
#include <smmintrin.h>
#include "nif_intrinsic.matkernel.h"
#include <stdint.h>
...
typedef struct nif_L4_cache_head {
...
typedef struct nif_L4_handle {
        void *ptr;
        nif_L4_cache_head* head;
} nif_L4_handle;

typedef struct matrix_task {
        nif_L4_handle buffer_0;
        nif_L4_handle buffer_1;
        nif_L4_handle buffer_2;
        nif_L4_handle buffer_3;
        union {
                struct {
                        int32_t reverse_0;
                        int32_t reverse_1;
...
typedef void(NIF_DECL *UNI_MK_PROC)(matrix_task *task);
typedef void(NIF_DECL *UNI_SK_PROC)(mat_kernel const *left, mat_kernel const *right,
                                   mat_kernel *restrict ans, uint64_t scalar);

typedef void(NIF_DECL *VECTORNORMALIZEPROC)(VEC_SEGMENT(float) *source, VEC_SEGMENT(float) *target);
typedef void(NIF_DECL *DOTPROC)(void *transpose, __m128 *vector, __m128 *ans);
typedef uint64_t(NIF_DECL *FACTORIALROC)(int n);

#pragma region string

typedef int(NIF_DECL *HASH)(byte const *str, int length);
typedef int(NIF_DECL *INDEX_PAIR)(short const *stram, uint32_t target, int length);
