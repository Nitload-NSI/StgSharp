...
INTERNAL void NIF_DECL f32_normalize(VEC_SEGMENT(float) *restrict source, VEC_SEGMENT(float) *restrict target)
{
        ...
        __m128 dot_product = _mm_mul_ps(target->f32_x[0], s);
        ...
        __m128 b_orthogonal = _mm_sub_ps(target->f32_x[0], _mm_mul_ps(s, dot_product));
        ...
        target->f32_x[0] = _mm_mul_ps(b_orthogonal, b_inv_length);
}
