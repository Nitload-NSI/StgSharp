...
typedef int (*nif_linux_id_visitor)(int value, void *context);

typedef struct nif_linux_counter_context {
        int count;
} nif_linux_counter_context;

typedef struct nif_linux_max_context {
        int max_value;
} nif_linux_max_context;

typedef struct nif_linux_fill_cpu_context {
        uint64_t *internal;
        uint32_t *global;
        int index;
} nif_linux_fill_cpu_context;

typedef struct nif_linux_fill_node_count_context {
        int *count_of_each_numa;
} nif_linux_fill_node_count_context;

static int nif_linux_read_first_line(char const *path, char *buffer, size_t buffer_size)
{
...
static int nif_linux_parse_nonnegative_int(char const **cursor, int *value)
{
        char *end = NULL;
...
static int nif_linux_visit_id_list(char const *text, nif_linux_id_visitor visitor, void *context)
{
        ...
        while (*cursor != '\0') {
                ...
                int begin = 0;
                if (!nif_linux_parse_nonnegative_int(&cursor, &begin))
                        return 0;
                ...
                if (*cursor == '-') {
                        cursor++;
                        if (!nif_linux_parse_nonnegative_int(&cursor, &finish))
                                return 0;
                }
...
static int nif_linux_count_id(int value, void *context)
{
        (void)value;
        ((nif_linux_counter_context *)context)->count++;
        return 1;
}

static int nif_linux_track_max_id(int value, void *context)
{
        nif_linux_max_context *max_context = (nif_linux_max_context *)context;
        if (value > max_context->max_value)
                max_context->max_value = value;
        return 1;
}

static int nif_linux_fill_cpu_id(int value, void *context)
{
        nif_linux_fill_cpu_context *fill_context = (nif_linux_fill_cpu_context *)context;
...
static int nif_linux_count_cpu_list(char const *path)
{
        char buffer[256];
        if (!nif_linux_read_first_line(path, buffer, sizeof(buffer)))
                return 0;

        nif_linux_counter_context counter = { 0 };
        if (!nif_linux_visit_id_list(buffer, nif_linux_count_id, &counter))
                return 0;

        return counter.count;
}

static int nif_linux_fill_node_cpu_count(int value, void *context)
{
        char path[128];
        snprintf(path, sizeof(path), "/sys/devices/system/node/node%d/cpulist", value);

        ((nif_linux_fill_node_count_context *)context)->count_of_each_numa[value] =
                nif_linux_count_cpu_list(path);
        return 1;
}

static int nif_linux_node_exists(int numa_id)
{
        char path[128];
...
static size_t nif_linux_round_up_to_page_size(size_t size)
{
...
static int nif_linux_bind_memory_to_node(void *address, size_t size, int numa_id, uint32_t flags)
{
...
static int nif_linux_populate_writable_mapping(void *address, size_t size)
{
...
{
        char buffer[256];
        if (!nif_linux_read_first_line("/sys/devices/system/node/online", buffer, sizeof(buffer)))
                return 0;

        nif_linux_max_context max_context = { -1 };
        if (!nif_linux_visit_id_list(buffer, nif_linux_track_max_id, &max_context) ||
        ...
        if (count_of_each_numa != NULL) {
                ...
                nif_linux_fill_node_count_context fill_context = { count_of_each_numa };
                if (!nif_linux_visit_id_list(buffer, nif_linux_fill_node_cpu_count, &fill_context))
                        return 0;
        }
...
NIF_API int NIF_DECL nif_get_numa_each(int numa_id, uint64_t *internal, uint32_t *global)
{
        ...
        nif_linux_fill_cpu_context fill_context = { internal, global, 0 };
        return nif_linux_visit_id_list(buffer, nif_linux_fill_cpu_id, &fill_context);
}

NIF_API void *NIF_DECL nif_numa_alloc(size_t size, int numa_id)
{
        size_t allocation_size = nif_linux_round_up_to_page_size(size);
        ...
        if (!nif_linux_node_exists(numa_id) ||
            !nif_linux_bind_memory_to_node(memory, allocation_size, numa_id, bind_flags) ||
            !nif_linux_populate_writable_mapping(memory, allocation_size)) {
