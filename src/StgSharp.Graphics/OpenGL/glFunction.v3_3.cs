...
namespace StgSharp.Graphics.OpenGL
{
    public unsafe partial class OpenGLFunction
    {
        ...
        {
            _context->glBlitFramebuffer(srcX0, srcY0, srcX1, srcY1, dstX0, dstY0, dstX1, dstY1,
                                        mask, filter);
        }
        ...
        {
            fixed (IntPtr* dataPtr = data) {
                _context->glCompressedTexImage2D(target, level, internalformat, width, height,
        ...
        {
            fixed (IntPtr* dataPtr = data) {
                _context->glCompressedTexImage3D(target, level, internalformat, width, height,
                                                 depth, border, imageSize, dataPtr);
            }
        }
        ...
        {
            fixed (IntPtr* dataPtr = data) {
                _context->glCompressedTexSubImage1D(target, level, xoffset, width, format,
                                                    imageSize, dataPtr);
            }
        }
        ...
        {
            fixed (IntPtr* indicesPtr = indices) {
                _context->glDrawElementsInstanced(mode, count, (uint)type, indicesPtr,
                                                  instancecount);
            }
        }
        ...
        )
        {
            fixed (uint* uniformIndicesPtr = uniformIndices) {
                _context->glGetActiveUniformsiv(program.Value, uniformCount, uniformIndicesPtr,
                                                pname, @params);
            }
        }
        ...
        {
            return Marshal.PtrToStringAuto((nint)_context->glGetStringi(name, index)) ??
                string.Empty;
        }
        ...
        {
            _context->glGetTransformFeedbackVarying(program.Value, index, bufSize, length, size,
                                                    type, name);
        }
        ...
        )
        {
            ...
            {
                fixed (IntPtr* indicesPtr = indices) {
                    _context->glMultiDrawElements(mode, countPtr, (uint)type, indicesPtr,
                                                  drawcount);
                }
            }
        }
        ...
        )
        {
            fixed (byte* varyingsPtr = varyings) {
                _context->glTransformFeedbackVaryings(program.Value, count, varyingsPtr,
                                                      bufferMode);
            }
        }
