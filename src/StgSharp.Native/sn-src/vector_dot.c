...
{
        *ans = _mm_dp_ps(transpose->x[0], *vector, 0B11110001);
        *ans = _mm_insert_ps(*ans, _mm_dp_ps(transpose->x[1], *vector, 0B11110001),
                             SSE_INSERT_SIMPLE(1, 1));
        *ans = _mm_insert_ps(*ans, _mm_dp_ps(transpose->x[2], *vector, 0B11110001),
                             SSE_INSERT_SIMPLE(1, 3));
        *ans = _mm_insert_ps(*ans, _mm_dp_ps(transpose->x[3], *vector, 0B11110001),
                                 ...
                                 __m128 *ans)
{
        ...
        __m256 tmp = _mm256_dp_ps(transpose->y[0], _double, 0B11110001);
        tmp = _mm256_blend_ps(tmp, _mm256_dp_ps(transpose->y[0], _double, 0B11110010),
                              0B00100010);
...
INTERNAL void SN_DECL dot_42_sse(MAT_KERNEL(float) const *transpose, __m128 const *vector,
                                 __m128 *ans)
{
        ans[0] = _mm_dp_ps(transpose->x[0], vector[0], 0B11110001);
        ans[0] = _mm_insert_ps(*ans, _mm_dp_ps(transpose->x[1], vector[0], 0B11110001),
                               SSE_INSERT_SIMPLE(1, 2));
        ans[0] = _mm_insert_ps(*ans, _mm_dp_ps(transpose->x[2], vector[0], 0B11110001),
                               SSE_INSERT_SIMPLE(1, 3));
        ans[0] = _mm_insert_ps(*ans, _mm_dp_ps(transpose->x[3], vector[0], 0B11110001),
                               SSE_INSERT_SIMPLE(1, 4));

        ans[1] = _mm_dp_ps(transpose->x[0], vector[1], 0B11110001);
        ans[1] = _mm_insert_ps(*ans, _mm_dp_ps(transpose->x[1], vector[1], 0B11110001),
                               SSE_INSERT_SIMPLE(1, 2));
        ans[1] = _mm_insert_ps(*ans, _mm_dp_ps(transpose->x[2], vector[1], 0B11110001),
                               SSE_INSERT_SIMPLE(1, 3));
        ans[1] = _mm_insert_ps(*ans, _mm_dp_ps(transpose->x[3], vector[1], 0B11110001),
                               SSE_INSERT_SIMPLE(1, 4));
}
