...
namespace Nitload.RegularAnalysis.Text
{
    ...
    internal readonly struct CaptureRange
    {

        private readonly uint _encodedStart;
        ...
        )
        {
            ...
            _encodedStart = (uint)start + 1u;
            Length = length;
        }

        public bool Success => _encodedStart != 0;

        public int Length { get; }

        public int Start => unchecked((int)(_encodedStart - 1u));
    ...
    public readonly ref struct MatchResult
    {
        ...
        private readonly CaptureRange[]? _groupRanges;
        private readonly FrozenDictionary<string, int>? _namedGroups;
        private readonly ReadOnlySpan<char> _text;

        internal MatchResult(
                 ReadOnlySpan<char> text,
                 FrozenDictionary<string, int> namedGroups,
                 CaptureRange[] groupRanges
        )
        {
            ArgumentNullException.ThrowIfNull(namedGroups);
            ArgumentNullException.ThrowIfNull(groupRanges);
        ...
        ]
        {
            get
            {
                ...
                if (_namedGroups is null || !_namedGroups.TryGetValue(name, out int index)) {
                    throw new KeyNotFoundException($"The match result does not define a group named '{name}'.");
                }

                CaptureRange range = _groupRanges![index];
                return range.Success ? _text.Slice(range.Start, range.Length) : default;
            }
        }
        ...
        )
        {
            ...
            if (_namedGroups is not null && _namedGroups.TryGetValue(name, out int index))
            {
                CaptureRange range = _groupRanges![index];
                if (range.Success)
                {
                    value = _text.Slice(range.Start, range.Length);
                    return true;
                }
            }
