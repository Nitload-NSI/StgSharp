    ...
    internal class RegexTokenReader : ITokenReader<RegexElementLabel>
    {
        ...
        public Token<RegexElementLabel> ReadToken()
        {
            ...
            switch (_source[pos])
            {
                ...
                case '\\':
                    ...
                    } else if (_source[pos + 1] == 'u')
                    {
                        const int unicodeEscapeLength = 6;
                        if (pos > _source.Length - unicodeEscapeLength)
                        {
                            throw new InvalidCastException(
                                "A Unicode escape must contain exactly four hexadecimal digits.");
                        }

                        ReadOnlySpan<char> value = _source.AsSpan(pos + 2, 4);
                        if (ushort.TryParse(value, NumberStyles.AllowHexSpecifier,
                                            CultureInfo.InvariantCulture, out ushort code))
                        {
                            position = pos + unicodeEscapeLength;
                            return new Token<RegexElementLabel>(((char)code).ToString(), 0, pos,
                                                                RegexElementLabel.UNIT);
                        } else
                        {
                            throw new InvalidCastException(
                                $"Failed to convert {value.ToString()} to a UTF-16 code unit.");
                        }
                    } else if (_source[pos + 1] == 'p')
                    {
                        int i = pos + 2;
                        if (_source[i] != '{') {
                            throw new InvalidOperationException("Invalid Unicode property escape sequence.");
                        }
        ...
        private Token<RegexElementLabel> ReadGroupBeginToken()
        {
            ...
            if (c2 == '<')
            {
                ...
                // Look behind prefixes: "(?<=" and "(?<!"
                if (c3 == '=')
                {
                    position = start + 4;
                    throw new NotSupportedException("Positive look behind '(?<=...)' is not supported yet.");
                }
                if (c3 == '!')
                {
                    position = start + 4;
                    throw new NotSupportedException("Negative look behind '(?<!...)' is not supported yet.");
                }
