...
namespace Nitload.Common.PipeLine
{
    public static class PipeLineRunner
    {
        ...
        public static void SetMaxTaskCount(
                           int size
        )
        {
            if (size < 1) {
        ...
        internal static bool InterruptAndRunNewPipeline(
                             PipelineScheduler newPipeline
        )
        {
            ArgumentNullException.ThrowIfNull(newPipeline);
            if ((newPipeline != NodeRunner.CurrentPipeline) && (NodeRunner.CurrentPipeline != null))
            {
