...
// using hlsfHandle = global::Nitload.Common.HighPerformance.Memory.HybridLayerSegregatedFitAllocationHandle;
...
namespace Nitload.Mathematics.Numeric
{
    public unsafe partial class Matrix<T> where T : unmanaged, INumber<T>
    {
        ...
        ]
        {
            ...
            {
                if ((y < 0) ||
                    (y > _size.ElementColumnLength) ||
                    (x < 0) ||
                    (x > _size.ElementRowLength)) {
        ...
        internal MatrixKernel<T>* Buffer => (MatrixKernel<T>*)_buffer.BufferPointer;
