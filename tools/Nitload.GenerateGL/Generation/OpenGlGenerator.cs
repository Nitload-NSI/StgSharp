...
namespace StgSharp.GenerateGL.Generation
{
    ...
    internal static class OpenGlGenerator
    {
        ...
        public static GeneratedFileSet Generate(
                                                ...
                                                GlGenerationTarget target = GlGenerationTarget.All
        )
        {
            ArgumentNullException.ThrowIfNull(projection);
            if (target == GlGenerationTarget.None || (target & ~GlGenerationTarget.All) != 0)
            {
                throw new ArgumentOutOfRangeException(nameof(target));
            }

            IReadOnlyList<ManagedEnumDefinition> enums = GlEnumProjector.Project(projection);
            GlEnumTypeMap enumTypeMap = new GlEnumTypeMap(enums);
            List<GeneratedFile> files = new List<GeneratedFile>();
            if ((target & GlGenerationTarget.Enum) != 0)
            {
                files.Add(new GeneratedFile("glconst.cs", GenerateConstants(projection.Enums)));
                files.Add(new GeneratedFile("GlEnums.g.cs", GenerateEnums(enums)));
            }

            if ((target & GlGenerationTarget.Api) != 0)
            {
                ...
                ManagedApiPlanner apiPlanner = new ManagedApiPlanner(
                    typeMapper,
                    familyAnalysis,
                    enumTypeMap);
        ...
        )
        {
            ...
            AppendGeneratedHeader(builder, "glconst.cs", "Desktop OpenGL constants");
        ...
        private static string GenerateEnums(
            IReadOnlyList<ManagedEnumDefinition> enums
        )
        {
            StringBuilder builder = new StringBuilder(512 * 1024);
            ...
            builder.Append("namespace StgSharp.Graphics.OpenGL\n");
            builder.Append("{\n");
            foreach (ManagedEnumDefinition value in enums)
            {
                builder.Append("    /// <summary>Native OpenGL group");
                builder.Append(value.NativeGroups.Count == 1 ? " " : "s ");
                builder.Append(string.Join(", ", value.NativeGroups));
                builder.Append(".</summary>\n");
                if (value.IsFlags)
                {
                    builder.Append("    [System.Flags]\n");
                }
                ...
                builder.Append(" : uint\n");
                builder.Append("    {\n");
                foreach (ManagedEnumMemberDefinition member in value.Members)
                {
                    builder.Append("        ");
                    builder.Append(member.Name);
                    builder.Append(" = ");
                    if (member.NativeValue.Name == "GL_NONE" &&
                        member.NativeValue.Comment ==
                    ...
                    {
                        builder.Append("unchecked((uint)glConst.");
                        builder.Append(ReadConstantName(member.NativeValue.Name));
                        builder.Append(')');
                    }
        ...
        {
            StringBuilder builder = new StringBuilder(512 * 1024);
            AppendGeneratedHeader(builder, outputFileName, description);
            builder.Append("using StgSharp.Mathematics.Numeric.Graphics;\n\n");
            ...
            for (int index = 0; index < arguments.Count; index++)
            {
                ...
                switch (arguments[index])
                {
                    case ParameterArgument parameterArgument:
                        ...
                        {
                            builder.Append("(uint");
                            builder.Append('*', parameter.Type.Count(value => value == '*'));
                            builder.Append(')');
                            builder.Append(parameterName);
                        }
                        else if (!parameter.IsHandle)
                        {
                            builder.Append(parameterName);
                        }
        ...
        )
        {
            builder.Append("// -----------------------------------------------------------------------------\n");
            builder.Append("// file=\"");
            builder.Append(ReadGeneratedFileName(outputFileName));
            builder.Append("\"\n");
            builder.Append("// Project: StgSharp\n");
            builder.Append("// Copyright (c) Nitload.\n");
            builder.Append("// SPDX-License-Identifier: MIT\n");
            builder.Append("// -----------------------------------------------------------------------------\n\n");
        ...
        private static string ReadGeneratedFileName(
            string outputFileName
        )
        {
            const string generatedExtension = ".g.cs";
            const string sourceExtension = ".cs";
            if (outputFileName.EndsWith(generatedExtension, StringComparison.Ordinal))
            {
                return outputFileName[..^generatedExtension.Length];
            }

            return outputFileName.EndsWith(sourceExtension, StringComparison.Ordinal)
                ? outputFileName[..^sourceExtension.Length]
                : outputFileName;
        }
