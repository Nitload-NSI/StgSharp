...
{
    [StructLayout(LayoutKind.Explicit, Size = 12, Pack = 16)]
    public struct vec3d : IEquatable<vec3d>
    {
        ...
        public static implicit operator float[](vec3d vec)
        {
            return new float[3] { vec.X, vec.Y, vec.Y };
        }

        public static implicit operator vec3d(float[] floatArray)
        {
            return new vec3d(floatArray[0], floatArray[1], floatArray[2]);
        }
    }
}
