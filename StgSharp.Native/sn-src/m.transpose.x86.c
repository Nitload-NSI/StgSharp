...
SN_MK_PROC_DECL_STD(float, sse, , transpose)
{
        ...
        register __m128 col2 = right->x[2];
        register __m128 col3 = right->x[3];
        ...
        ans->x[0] = _mm_movelh_ps(tmp0, tmp2);
        ans->x[1] = _mm_movehl_ps(tmp2, tmp0);
        ans->x[2] = _mm_movelh_ps(tmp1, tmp3);
        ans->x[3] = _mm_movehl_ps(tmp3, tmp1);
}
...
SN_MK_PROC_DECL_STD(float, avx, , transpose)
{
        ...
        ans->y[0] = _mm256_shuffle_ps(col_high, col_low, _MM_SHUFFLE(2, 0, 2, 0));
        ans->y[1] = _mm256_shuffle_ps(col_high, col_low, _MM_SHUFFLE(3, 1, 3, 1));
}
...
SN_MK_PROC_DECL_STD(float, 512, , transpose)
{
        ...
        ans->z[0] = _mm512_permutexvar_ps(transpose_indices, matrix);
}
