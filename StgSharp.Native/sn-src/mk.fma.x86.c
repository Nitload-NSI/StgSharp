...
SN_MK_PROC_DECL_STD(float, sse, , fma)
{
        ...
        for (size_t z = 0; z < common_z; z++) {
                ...
                const __m128 a0 = _mm_load_ps((float *)&left_cur->x[0]);
                const __m128 a1 = _mm_load_ps((float *)&left_cur->x[1]);
                const __m128 a2 = _mm_load_ps((float *)&left_cur->x[2]);
                const __m128 a3 = _mm_load_ps((float *)&left_cur->x[3]);

                /* Column 0 */
                bcol = _mm_load_ps((float *)&right_cur->x[0]);
                ...
                /* Column 1 */
                bcol = _mm_load_ps((float *)&right_cur->x[1]);
                ...
                /* Column 2 */
                bcol = _mm_load_ps((float *)&right_cur->x[2]);
                ...
                /* Column 3 */
                bcol = _mm_load_ps((float *)&right_cur->x[3]);
...
SN_MK_PROC_DECL_STD(float, avx, , fma)
{
        ...
        register __m256 c01 = _mm256_load_ps((float *)&ans_cur->y[0]);
        register __m256 c23 = _mm256_load_ps((float *)&ans_cur->y[1]);

        for (size_t z = 0; z < common_z; z++) {
                SN_MAT_KERNEL_LOCAL const *right_cur = right + GET_INDEX(common_z, x, z);
                SN_MAT_KERNEL_LOCAL const *left_cur = left + GET_INDEX(common_y, z, y);

                register __m256 b01 = _mm256_load_ps((float *)&right_cur->y[0]);
                register __m256 b23 = _mm256_load_ps((float *)&right_cur->y[1]);

                register __m256 a0 = _mm256_broadcast_ps(&left_cur->x[0]);
                c01 = _mm256_add_ps(_mm256_mul_ps(a0, b01), c01);
                c23 = _mm256_add_ps(_mm256_mul_ps(a0, b23), c23);
                register __m256 a1 = _mm256_broadcast_ps(&left_cur->x[1]);
                c01 = _mm256_add_ps(_mm256_mul_ps(a1, b01), c01);
                c23 = _mm256_add_ps(_mm256_mul_ps(a1, b23), c23);
                register __m256 a2 = _mm256_broadcast_ps(&left_cur->x[2]);
                c01 = _mm256_add_ps(_mm256_mul_ps(a2, b01), c01);
                c23 = _mm256_add_ps(_mm256_mul_ps(a2, b23), c23);
                register __m256 a3 = _mm256_broadcast_ps(&left_cur->x[3]);
                c01 = _mm256_add_ps(_mm256_mul_ps(a3, b01), c01);
                c23 = _mm256_add_ps(_mm256_mul_ps(a3, b23), c23);
        }
        _mm256_store_ps((float *)&ans_cur->y[0], c01);
        _mm256_store_ps((float *)&ans_cur->y[1], c23);
}
...
SN_MK_PROC_DECL_STD(float, avx, _fma, fma)
{
        ...
        for (size_t z = 0; z < common_z; z+=2) {
                const SN_MAT_KERNEL_LOCAL *a_base = left + z;
                const SN_MAT_KERNEL_LOCAL *b_base = right + z;

                register __m256 b0 = _mm256_load_ps((float *)&b_base[0].z[0]);
                register __m256 b1 = _mm256_load_ps((float *)&b_base[1].z[0]);
                register __m256 b2 = _mm256_load_ps((float *)&b_base[2].z[0]);
                register __m256 b3 = _mm256_load_ps((float *)&b_base[3].z[0]);

                register __m256 a0 = _mm256_load_ps((float *)&a_base[0].z[0]);
                register __m256 a1 = _mm256_load_ps((float *)&a_base[1].z[0]);
                register __m256 a2 = _mm256_load_ps((float *)&a_base[2].z[0]);
                register __m256 a3 = _mm256_load_ps((float *)&a_base[3].z[0]);
        ...
        SN_FMA_AVX2_PACK_COLUMN(c0, (float*)&(ans_cur->x[0]));
        SN_FMA_AVX2_PACK_COLUMN(c1, (float*)&(ans_cur->x[1]));
        SN_FMA_AVX2_PACK_COLUMN(c2, (float*)&(ans_cur->x[2]));
        SN_FMA_AVX2_PACK_COLUMN(c3, (float*)&(ans_cur->x[3]));
}
...
SN_MK_PROC_DECL_STD(float, 512, , fma)
{
        ...
        for (size_t z = 0; z < common_z; z+=4) {
                ...
                register __m512 b0 = _mm512_load_ps((float *)&b_base[0].z[0]);
                register __m512 b1 = _mm512_load_ps((float *)&b_base[1].z[0]);
                register __m512 b2 = _mm512_load_ps((float *)&b_base[2].z[0]);
                register __m512 b3 = _mm512_load_ps((float *)&b_base[3].z[0]);

                register __m512 a0 = _mm512_load_ps((float *)&a_base[0].z[0]);
                register __m512 a1 = _mm512_load_ps((float *)&a_base[1].z[0]);
                register __m512 a2 = _mm512_load_ps((float *)&a_base[2].z[0]);
                register __m512 a3 = _mm512_load_ps((float *)&a_base[3].z[0]);
        ...
        SN_FMA_512_PACK_COLUMN(c0, (float*)&(ans_cur->x[0]));
        SN_FMA_512_PACK_COLUMN(c1, (float*)&(ans_cur->x[1]));
        SN_FMA_512_PACK_COLUMN(c2, (float*)&(ans_cur->x[2]));
        SN_FMA_512_PACK_COLUMN(c3, (float*)&(ans_cur->x[3]));
}
...
SN_MK_PROC_DECL_STD(double, avx, , fma)
{
        ...
        for (size_t z = 0; z < common_z; z++) {
                ...
                const __m256d a0 = _mm256_load_pd((double *)&left_cur->y[0]);
                const __m256d a1 = _mm256_load_pd((double *)&left_cur->y[1]);
                const __m256d a2 = _mm256_load_pd((double *)&left_cur->y[2]);
                const __m256d a3 = _mm256_load_pd((double *)&left_cur->y[3]);

                /* Column 0 */
                bcol = _mm256_load_pd((double *)&right_cur->y[0]);
                c0 = _mm256_add_pd(c0,
                ...
                /* Column 1 */
                bcol = _mm256_load_pd((double *)&right_cur->y[1]);
                c1 = _mm256_add_pd(c1,
                                   _mm256_mul_pd(a0, _mm256_permute4x64_pd(bcol, _FMA_SELECT(0))));
                c1 = _mm256_add_pd(c1,
                                   _mm256_mul_pd(a1, _mm256_permute4x64_pd(bcol, _FMA_SELECT(1))));
                ...
                /* Column 2 */
                bcol = _mm256_load_pd((double *)&right_cur->y[2]);
                c2 = _mm256_add_pd(c2,
                                   _mm256_mul_pd(a0, _mm256_permute4x64_pd(bcol, _FMA_SELECT(0))));
                c2 = _mm256_add_pd(c2,
                                   _mm256_mul_pd(a1, _mm256_permute4x64_pd(bcol, _FMA_SELECT(1))));
                ...
                /* Column 3 */
                bcol = _mm256_load_pd((double *)&right_cur->y[3]);
                c3 = _mm256_add_pd(c3,
                                   _mm256_mul_pd(a0, _mm256_permute4x64_pd(bcol, _FMA_SELECT(0))));
                c3 = _mm256_add_pd(c3,
                                   _mm256_mul_pd(a1, _mm256_permute4x64_pd(bcol, _FMA_SELECT(1))));
...
SN_MK_PROC_DECL_STD(double, 512, , fma)
{
        ...
        register __m512d c01 = _mm512_load_pd((double *)&ans_cur->z[0]);
        register __m512d c23 = _mm512_load_pd((double *)&ans_cur->z[1]);

        for (size_t z = 0; z < common_z; z++) {
                SN_MAT_KERNEL_LOCAL const *right_cur = right + GET_INDEX(common_z, x, z);
                SN_MAT_KERNEL_LOCAL const *left_cur = left + GET_INDEX(common_y, z, y);

                register __m512d a01 = _mm512_load_pd((double *)&left_cur->z[0]);
                register __m512d a23 = _mm512_load_pd((double *)&left_cur->z[1]);

                register __m512d b0 = _mm512_broadcast_f64x4(right_cur->y[0]);
                c01 = _mm512_fmadd_pd(a01, b0, c01);
                c23 = _mm512_fmadd_pd(a23, b0, c23);

                register __m512d b1 = _mm512_broadcast_f64x4(right_cur->y[1]);
                c01 = _mm512_fmadd_pd(a01, b1, c01);
                c23 = _mm512_fmadd_pd(a23, b1, c23);

                register __m512d b2 = _mm512_broadcast_f64x4(right_cur->y[2]);
                c01 = _mm512_fmadd_pd(a01, b2, c01);
                c23 = _mm512_fmadd_pd(a23, b2, c23);

                register __m512d b3 = _mm512_broadcast_f64x4(right_cur->y[3]);
                c01 = _mm512_fmadd_pd(a01, b3, c01);
                c23 = _mm512_fmadd_pd(a23, b3, c23);
        }
        _mm512_store_pd((double *)&ans_cur->z[0], c01);
        _mm512_store_pd((double *)&ans_cur->z[1], c23);
}
#endif
