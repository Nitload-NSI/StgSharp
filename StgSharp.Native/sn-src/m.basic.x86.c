...
SN_MK_PROC_DECL_STD(float, sse, , transpose)
{
        ...
        _mm_store_ps(&(ans->f32_x[0]), _mm_movelh_ps(tmp0, tmp2));
        _mm_store_ps(&(ans->f32_x[1]), _mm_movehl_ps(tmp2, tmp0));
        _mm_store_ps(&(ans->f32_x[2]), _mm_movelh_ps(tmp1, tmp3));
        _mm_store_ps(&(ans->f32_x[3]), _mm_movehl_ps(tmp3, tmp1));
}
...
SN_MK_PROC_DECL_STD(float, avx, , transpose)
{
        ...
        _mm256_store_ps(&(ans->f32_y[0]),
                        _mm256_shuffle_ps(col_high, col_low, _MM_SHUFFLE(2, 0, 2, 0)));
        _mm256_store_ps(&(ans->f32_y[1]),
                        _mm256_shuffle_ps(col_high, col_low, _MM_SHUFFLE(3, 1, 3, 1)));
}
...
SN_MK_PROC_DECL_STD(float, 512, , transpose)
{
        ...
        _mm512_store_ps(&(ans->f32_z[0]), _mm512_permutexvar_ps(transpose_indices, matrix));
}
